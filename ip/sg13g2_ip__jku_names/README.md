...
A names IP block for [Johannes Kepler University Linz](https://www.jku.at/), rendered as a GDSII layout on a configurable metal layer (default `Metal5`) with `NoMetFiller` for the IHP SG13G2 130 nm CMOS process.
...
```sh
make klayout-drc
make klayout-drc CELL=sg13g2_ip__jku_names
```
...
```sh
make magic-drc
make magic-drc CELL=sg13g2_ip__jku_names
```
