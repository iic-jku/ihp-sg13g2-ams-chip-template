...
| `make export-gds`                                 | Export the tapeout GDS of a Magic layout (`layout/magic/<TOP>.mag` to `layout/magic/<TOP>.gds`). A KLayout layout is exported from the GUI instead. |
...
| [`layout/`](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/tree/main/macros/inverter/layout/)         | Layout sources, one folder per drawing tool: `klayout/` with the KLayout sources (`*.klay.gds`, `*.klay.klib`) and the exported `inverter_top.gds`, and `magic/` for a layout drawn in Magic instead. |
...
The layout sources live in one folder per drawing tool, `layout/klayout/` and `layout/magic/`, the same way the schematics live in `schematic/xschem/`. This inverter is created and edited in KLayout, so `layout/magic/` holds only its README. The container provides the `ke` alias for `klayout -e` (edit mode). With the following command, the GDS of the 4-channel inverter is opened in KLayout as shown in @fig-inverter-top-layout. With the keys `0`, `1`, `2`, ..., `9` you can view different layers as explained in the [KLayout cheatsheet](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/blob/main/doc/klayout/klayout_cheatsheet.md) or in the [klayout-layer-shortcuts](https://github.com/iic-jku/klayout-layer-shortcuts) plugin.

```sh
ke layout/klayout/inverter_top.klay.gds
```

`inverter_top.klay.gds` is the source of truth. After modifying the layout, export a clean `.gds` file (for example `inverter_top.gds`) by pressing `File > Export Layout For Tapeout`, which resolves every PCell and library reference into a static cell so the result opens without the PDK loaded. Never hand-edit the exported file: keeping the two in step by editing both is how they drift apart, and the drift is invisible because the sign-off targets only look at the exported one. All verification flows (DRC, LVS, and PEX with both KLayout and Magic) accept either `.gds` or `.klay.gds`, while the build targets (LEF export, GDS copy, and rendering) use the exported `.gds`. No target names a layout folder: the Makefile searches `layout/klayout/` and then `layout/magic/`, so a macro drawn in Magic instead runs through the same flow, with `make export-gds` writing `layout/magic/<TOP>.gds` in place of the KLayout export.
...
```sh
make check-boundary   # check the PR boundary box on layer 189 in layout/klayout/inverter_top.gds
...
6. [Optional] Once the schematic results are convincing, propagate the resistor to the layout in [`layout/klayout/`](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/tree/main/macros/inverter/layout/klayout/) and re-run the verification chain (`make klayout-verify`, `make magic-verify`) plus `make build-top`.
