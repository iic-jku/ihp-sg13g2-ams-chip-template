...
The container also ships with four PDKs: [SkyWater sky130A](https://github.com/rtimothyedwards/open_pdks), [GlobalFoundries gf180mcuD](https://github.com/rtimothyedwards/open_pdks), [IHP ihp-sg13cmos5l](https://github.com/IHP-GmbH/ihp-sg13cmos5l) and [IHP ihp-sg13g2](https://github.com/IHP-GmbH/IHP-Open-PDK) used by this template.
...
Every `Makefile` in this repository guards against a mismatch. All targets except `help` and `clean` (plus `clean-all` at the top level) compare `$PDK` against `REQUIRED_PDK`, which defaults to `ihp-sg13g2`, and abort before any tool starts:
...
| `make klayout-pex` / `magic-pex` (`EXT_MODE=<1\|2\|3>`) | Extract the parasitics of the hardened macro from `final/gds/` into `netlist/pex/`. |
...
The counter design files are located in [`macros/counter/`](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/tree/main/macros/counter/), and the relevant folders are:
...
| [`fpga/`](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/tree/main/macros/counter/fpga/)           | Optional FPGA emulation flow for three boards (pico-ice, iCEBreaker, ULX3S).    |
...
The LibreLane render is copied to [`render/img/counter_top_librelane.png`](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/tree/main/macros/counter/render/img/), and the high-resolution renders from `sak-render.py` are saved as `counter_top_white.png` and `counter_top_black.png` in the same folder. The verification reports are written to [`verification/`](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/tree/main/macros/counter/verification/).
...
- Parasitic Extraction ([PEX](https://en.wikipedia.org/wiki/Parasitic_extraction)) with KLayout or Magic,
...
| Folder                                          | Content                                                                                  |
| ------------------------------------------- | ---------------------------------------------------------------------------------------- |
| [`schematic/xschem/`](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/tree/main/macros/inverter/schematic/xschem/) | Xschem schematics and symbols (`inverter.sch`, `inverter.sym`, `inverter_top.sch`, `inverter_top.sym`, and the generated `inverter_top_pex.sym`). |
| [`layout/`](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/tree/main/macros/inverter/layout/)         | The unit cell `inverter.gds`, the editing source `inverter_top.klay.gds` with its library map `inverter_top.klay.klib`, and the exported `inverter_top.gds`. |
...
| [`scripts/`](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/tree/main/macros/inverter/scripts/)       | Sizing notebooks (`sizing/`), the PR boundary check (`check_boundary.py`), and the PEX port check (`check_pex_ports.py`). |
...
We look at the ac testbench in detail, because it contains every ingredient a complete analog testbench needs. Open [`inverter_tb_ac_ol.sch`](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/blob/main/macros/inverter/testbenches/xschem/inverter_tb_ac_ol.sch) (see @fig-inverter-tb-ac-ol-schematic) and identify the following parts:
...
**7. The data export.** The final `wrdata` command writes the traces to [`plot_simulations/data/`](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/tree/main/macros/inverter/testbenches/xschem/plot_simulations/data), so the Python plotting script and the batch flow from @sec-batch-sim can use them.
...
To draw this schematic from scratch, start Xschem in `macros/inverter/schematic/xschem/` and go through the following steps. The reference is [`inverter.sch`](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/blob/main/macros/inverter/schematic/xschem/inverter.sch), and the device geometry comes from the sizing in @sec-sizing (see @tbl-inverter-sizing-geom).

1. **Place the transistors.** Press `Ins`, and pick `sg13_lv_nmos.sym` and `sg13_lv_pmos.sym` from the `sg13g2_pr` PDK library. Place the NMOS $M_1$ at the bottom and the PMOS $M_2$ at the top.
...
To use the inverter in a testbench or in the 4-channel `inverter_top.sch`, it needs a symbol. @fig-inverter-symbol shows the finished symbol, the reference is [`inverter.sym`](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/blob/main/macros/inverter/schematic/xschem/inverter.sym).
...
The layout is created and edited in KLayout. Open it in edit mode with `ke`, an alias for `klayout -e` provided by the container. With the ihp-sg13g2 PDK active, KLayout loads the `sg13g2` technology, which brings the layer properties, the `SG13_dev` PCell library, and the `SG13G2 PDK` menu with the DRC and LVS runners.
...
- **Database unit.** When creating a new layout (`File > New Layout`), set the database unit (DBU) to $0.001\,\mathrm{\mu m}$. This is important for the IHP Open-PDK. The [KLayout cheatsheet](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/blob/main/doc/klayout/klayout_cheatsheet.md) explains how to rescale a layout that was started with the wrong DBU.
...
- **PCells.** The PDK provides parametric cells (PCells) in the `SG13_dev` library, for example `nmos`, `pmos`, resistors (`rppd`, `rsil`, `rhigh`), capacitors (`cmim`, `moscap_n`, `moscap_p`), taps (`ntap1`, `ptap1`), the `guard_ring`, and `via_stack`. Select a placed PCell and press `Q` to edit its parameters.

The most important shortcuts are listed in @tbl-klayout-keys, taken from the [KLayout cheatsheet](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/blob/main/doc/klayout/klayout_cheatsheet.md) in this repository.
...
After the schematic is finalised and the design targets are met, the layout can be created. The layout of the `inverter` cell is hierarchical, and the top-level cell `inverter_top` contains four instances of the `inverter` cell. Around the 4-channel inverter layout, a power ring is drawn on `Metal5` and `TopMetal1` layers, which is connected to the `VDD` and `VSS` pins of the four inverters. This power ring is required for top-level integration, connecting the macro power to the chip's PDN.
...
4. **Substrate and well contacts.** Tie the substrate to `VSS` and the n-well of the PMOS to `VDD` with `ptap1` / `ntap1` contacts (or a `guard_ring`).
...
::: {.callout-tip}
The layout drawing order follows the matching rules of analog design: active devices in the centre with common orientation, dummies at the edges, and taps close to the devices. Also check the layout tips in the [KLayout cheatsheet](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/blob/main/doc/klayout/klayout_cheatsheet.md).
:::
...
After the reorder, both PEX targets run [`scripts/check_pex_ports.py`](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/blob/main/macros/inverter/scripts/check_pex_ports.py) on the extracted netlist and fail if a port of the subcircuit is not connected to any element inside it. A floating port usually means a missing or misspelled pin label in the layout, and it is better caught here than as a silently open input in the post-layout simulation.
...
4. The `.include ../../../netlist/pex/inverter_magic_pex_3.spice` line at the top of the control block now supplies the subcircuit definition. If you extracted with a different `EXT_MODE` or with KLayout, adapt the file name (for example `inverter_klayout_pex_3.spice`).
...
Geometry and metal stack are parameterised through three Makefile variables. The defaults match the chip's padframe and the SG13G2 PDK defaults.
...
The artifacts are written to [`release/v.2.1.0/`](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/tree/main/release/) (`gds/`, `netlist/{schematic,layout,pnl,spice}/`, `img/`).
...
1. Open the inverter schematic ([`inverter.sch`](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/blob/main/macros/inverter/schematic/xschem/inverter.sch)) in Xschem and add a resistor between the `vin` and `vout` nets of the inverter. Use the `rhigh.sym` device from the `sg13g2_pr` PDK library, the high-sheet poly resistor. Keep `spiceprefix=X` and connect its `body` terminal to `VSS`. The symbol computes the value from the geometry: with a sheet resistance of $1360\,\mathrm{\Omega/sq}$ this is roughly $2.7\,\mathrm{k\Omega}$ per $\mathrm{\mu m}$ of length at $W = 0.5\,\mathrm{\mu m}$, so keep $W$ at $0.5\,\mathrm{\mu m}$ and set the value with $L$ (the `b` parameter folds a long resistor into segments). Start with a value of $R_\mathrm{f} = 1\,\mathrm{M\Omega}$ and adjust as needed. What can you observe?
...
1. Start the new macro as a renamed copy of the [inverter](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/tree/main/macros/inverter/) with `make macro FROM=inverter NAME=ringosc` in `macros/`. The target renames the files, the cells inside the GDS files and every reference, see the [macros README](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/blob/main/macros/README.md) for what it does and what is left.
...
4. Update the transient testbench ([`testbenches/xschem/inverter_top_tb_tran.sch`](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/blob/main/macros/inverter/testbenches/xschem/inverter_top_tb_tran.sch)) to measure the oscillation frequency. Remove the input source. Since the delay per stage is small, simulate a few hundred nanoseconds with a fine step, and measure the period between two consecutive rising crossings of $V_\mathrm{cm}$ on the output net:
...
6. [Optional] Adapt the CACE [`inverter.yaml`](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/blob/main/macros/inverter/verification/cace/inverter.yaml) to characterise the oscillation frequency across PVT.
