...
::: {.callout-important}
This tutorial and the underlying template repository require the [IIC-OSIC-TOOLS](https://github.com/iic-jku/IIC-OSIC-TOOLS) container with tag `2026.06` or later. All commands below must be executed from within the container, and the working directory must be a folder containing the corresponding `Makefile`.
:::
...
Solid arrows represent direct `$(MAKE) <target>` calls within a single Makefile. Dashed arrows represent recursive `$(MAKE) -C <dir> all` calls that descend into a sub-Makefile (one per coloured deliverable).

![Overview of the Makefile targets that `make all` resolves to. Every coloured branch corresponds to one deliverable (top chip, bondpad, logos, digital macro, analog macro). The grey targets connect `make all` to those branches.](fig/targets_overview/targets_overview.svg){#fig-targets-overview}
