...
::: {.callout-important}
This tutorial and the underlying template repository require the [IIC-OSIC-TOOLS](https://github.com/iic-jku/IIC-OSIC-TOOLS) container with tag `2026.05` or later. All commands below must be executed from within the container, and the working directory must be a folder containing the corresponding `Makefile`.
:::
...
The colour scheme groups every target by the deliverable it ultimately produces, so a beginner can trace a colour from `make all` down to the final outputs:

- **Orange — top chip / main**: `make all` itself and every target that contributes to the top-level chip GDS (the `build-top` chain in the [top-level Makefile](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/blob/main/Makefile): `librelane-nodrc`, `copy-*`, `add-logo-fill`, `render-gds`).
- **Yellow — bondpad IP**: `build-bondpad` and everything inside [`ip/sg13g2_ip__bondpad_70x70/`](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/tree/main/ip/sg13g2_ip__bondpad_70x70/).
- **Purple — logos IP**: `build-logos` and the two sub-Makefiles under [`ip/sg13g2_ip__jku/`](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/tree/main/ip/sg13g2_ip__jku/) and [`ip/sg13g2_ip__jku_names/`](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/tree/main/ip/sg13g2_ip__jku_names/).
- **Blue — digital macro**: `build-counter` and the [`macros/counter/`](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/tree/main/macros/counter/) Makefile. Blue is the conventional colour for digital design.
- **Green — analog macro**: `build-inverter` and the [`macros/inverter/`](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/tree/main/macros/inverter/) Makefile. Green is the conventional colour for analog design.
- **Grey — simulation & verification**: intermediate targets that fan out into the coloured deliverables (`sim-all`, `build-all`, `build-macros`, `init-submodules`, `sim-*-cocotb`) and the top-level `magic-drc` checks that are run after the chip is built.

Solid arrows are direct `$(MAKE) <target>` calls within a single Makefile; dashed arrows are recursive `$(MAKE) -C <dir> all` calls that descend into a sub-Makefile (one per coloured deliverable).

![Overview of the Makefile targets that `make all` resolves to. Every coloured lane corresponds to one deliverable (top chip, bondpad, logos, digital macro, analog macro). The grey targets connect `make all` to those lanes.](fig/targets_overview/targets_overview.svg){#fig-targets-overview}
...
| `klayout-drc` | Run KLayout DRC (`make klayout-drc [CELL=<cellname>]`).                                                                                                                                                 |
...
Both logo IPs share the same Makefile targets and PNG-to-GDS converter [`script/make_gds.py`](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/blob/main/ip/sg13g2_ip__jku/script/make_gds.py), built on the `klayout.db` Python module. The conversion proceeds in five steps:
...
| Target        | What it does                                                                                       |
| ------------- | -------------------------------------------------------------------------------------------------- |
| `logo`        | Convert the PNG into a GDSII layout using [`make_gds.py`](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/blob/main/ip/sg13g2_ip__jku/script/make_gds.py). |
...
| `klayout-drc` | Run KLayout DRC (same relaxed rule-set as the bondpad).                                            |
...
::: {.callout-note}
The top-level Xschem testbench converges, but it may take a long time depending on the hardware used. `make sim-gl-xschem` is therefore not part of `sim-all` and must be called manually when needed. For this tutorial, the simulation data is already available in the repository in [`scripts/plot_simulations/data/chip_top_tb_tran.txt`](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/blob/main/scripts/plot_simulations/data/chip_top_tb_tran.txt), so `make plot-xschem-sim TESTBENCH=chip_top_tb_tran` can be run without running the simulation first.
:::
