...
::: {.callout-warning}
At top level, PEX is only run on `chip_top` (without logo and filler), and the default `EXT_MODE=1` (C-decoupled) is used to keep the runtime manageable. However, the extracted netlist is still huge, and the post-layout simulation must be run on a powerful server.
:::
