...
After modifying the layout, export a clean `.gds` file (for example `inverter_top.gds`) by pressing `File > Export Layout For Tapeout`. The Magic-based LVS flow only works on `.gds` files, while the KLayout-based verification flows as well as the Magic DRC and PEX flows accept either `.gds` or `.klay.gds`.
