...
After modifying the layout, export a clean `.gds` file (for example `inverter_top.gds`) by pressing `File > Export Layout For Tapeout`. The Magic-based LVS and PEX flows only work on `.gds` files, while the KLayout-based verification flows and the Magic DRC flow accept either `.gds` or `.klay.gds`.
