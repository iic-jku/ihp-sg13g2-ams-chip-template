...
::: {.callout-tip}
The macro can also be built as an FPGA bitstream ([`fpga/`](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/tree/main/macros/counter/fpga/)) with `make build-fpga`. `counter_top` is synthesized straight onto board pins, without a board-top wrapper, and six boards are supported across four FPGA architectures: pico-ice and iCEBreaker (Lattice iCE40), ULX3S (Lattice ECP5), Tang Nano 9K (Gowin) and Basys 3 and Boolean (Xilinx 7-series). Pick one with `make build-fpga` for the default pico-ice, or `make -C fpga BOARD=ulx3s all` for another. IIC-OSIC-TOOLS carries the complete iCE40 chain, so the two iCE40 boards build end to end inside the container, while the others need extra tools listed in [`fpga/README.md`](https://github.com/iic-jku/ihp-sg13g2-ams-chip-template/blob/main/macros/counter/fpga/README.md). The FPGA flow is not used in this tutorial and is only listed here for reference.
:::
