...
def ensure_dir(path):
    """Create the parent directory of *path*, so a fresh tree needs no mkdir."""
...
def generate_package(cfg):
    ...
    if not cfg["PACKAGE_GDS"]:              # default: next to the library
        cfg["PACKAGE_GDS"] = os.path.join(os.path.dirname(path),
                                          name + ".gds")
    dest = ensure_dir(resolve(cfg, cfg["PACKAGE_GDS"]))
    out.write(dest)
...
def _write_markdown_report(cfg, rows, stats, out):
    ...
    lines += ["Generated by `scripts/run_bondplan.py` on %s." % stats["date"],
              ""]
    if cfg["BONDPLAN_PNG"]:
        png = os.path.splitext(ensure_dir(resolve(cfg, cfg["BONDPLAN_PNG"])))[0] + \
            "_white.png"
    ...
    with open(ensure_dir(out), "w") as f:
        f.write("\n".join(lines) + "\n")
...
def export_png(cfg, gds_path, crop):
    ...
    base, ext = os.path.splitext(ensure_dir(resolve(cfg, cfg["BONDPLAN_PNG"])))
...
def export_svg(cfg, ly, top, crop):
    ...
    width, height = image_size(cfg, crop)
    base, ext = os.path.splitext(ensure_dir(resolve(cfg, cfg["BONDPLAN_SVG"])))
    for suffix, bg in (("_white", "#FFFFFF"), ("_black", "#000000")):
