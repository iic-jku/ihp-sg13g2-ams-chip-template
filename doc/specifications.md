...
This document describes the AMS template chip from a designer's point of
view: what it contains, how it behaves, and where each subsystem is
configured. The numbers below are read from the actual design files; if you
change them in the source, please mirror them here.
...
Top-level RTL:
- [rtl/chip_top.sv](../rtl/chip_top.sv)  — padframe wrapper
- [rtl/chip_core.sv](../rtl/chip_core.sv) — digital + analog macros + SRAM
...
The padframe uses the generic top-level port names defined in
`chip_top.sv`. The mapping of bit indices to roles is in `chip_core.sv`.
...
Full per-pad breakdown including pad-cell instance names is in
[pinout.md](pinout.md).
...
`input_PAD[0]` (also named `enable` inside `chip_core.sv`) is the
chip-wide enable signal. It gates:
...
mode). `counter2_value[7:0]` drives `output_PAD[11:4]` (full south side).

### Inverter 1 — digital

`inverter1` (instance of `inverter_top`) is used as a 4-channel CMOS
digital inverter:

- `vin1..vin4` come from `bidir_in[3:0]` (i.e. `counter1_value[7:4]` when
  `enable = 1`, or external stimuli when `enable = 0`).
- `vout1..vout4` drive `output_PAD[15:12]` on the east side.

Two test modes are therefore possible:

1. **Coupled to counter1** (`enable = 1`): observe `~counter1_value[7:4]`
   on `output_PAD[15:12]` while `counter1_value[7:4]` itself is observable
   on `bidir_PAD[3:0]`.
2. **Standalone** (`enable = 0`): drive `bidir_PAD[3:0]` externally and
   observe the inverted response on `output_PAD[15:12]`.

### Inverter 2 — analog

`inverter2` (instance of `inverter_top`) is used as an analog block. Two of
its four channels are exposed via four east-side analog pads:
...
Channels 3 and 4 (`vin3`, `vout3`, `vin4`, `vout4`) are tied to `VSS`
inside `chip_core.sv` and are not bonded out.

The `padres` path goes through the IO pad's secondary ESD resistor; the
`padbare` path bypasses it (lower parasitics, used for the output
signals). The corresponding nets are listed in `RSZ_DONT_TOUCH_LIST` in
`config.yaml` so the resizer leaves them alone, and the `DRT_ASSIGN_NDR`
regex applies the wider `NDR_analog` non-default rule to them during
detailed routing.

### SRAM

The `RM_IHPSG13_1P_1024x32_c2_bm_bist` macro is wired up for a simple
read-only demonstration in `chip_core.sv`:
...
The behavioural model has no `USE_POWER_PINS` block, so the Verilog
instantiation only connects the functional/BIST ports. Physical supply
hookup is handled at the PDN level via `PDN_MACRO_CONNECTIONS` in
`flow/librelane/config.yaml`, which binds both the macro's `VDD!/VSS!` and
its secondary `VDDARRAY!/VSS!` to the chip's `VDD`/`VSS`.


## Power and Reset

- One IOVDD/IOVSS pair powers the I/O ring. Filler pads in the padframe
  distribute the supply around the four sides.
- One VDD/VSS pair powers the core. The chip's PDN ring sits on TopMetal1
  (horizontal) and TopMetal2 (vertical), 15 µm wide, with 5 µm spacing.
- Each `inverter_top` instance has its own internal power ring on Metal5
  (horizontal) and TopMetal1 (vertical). A custom PDN grid bridges the
  chip ring to the macro ring (`flow/librelane/pdn_cfg.tcl`).
- The SRAM has its own PDN grid (`sram_NS` block in `pdn_cfg.tcl`) with
  M5 stripes that connect to Metal4 and TopMetal1.
- Active-low reset on `rst_n_PAD`. The counter macro converts polarity
  internally and feeds a synchronous active-high reset to its FFs.
...
| Topic                    | File                                                                                |
| ------------------------ | ----------------------------------------------------------------------------------- |
| Top-level RTL            | [rtl/chip_top.sv](../rtl/chip_top.sv), [rtl/chip_core.sv](../rtl/chip_core.sv)      |
